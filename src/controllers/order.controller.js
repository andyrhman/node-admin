...
const Orders = async (req, res) => {
    ...
    let result = await repository.paginate(page, take, { model: OrderItem, as: 'order_item' });
...
const Export = async (req, res) => {
    ...
    const orders = await Order.findAll({ include: { model: OrderItem, as: 'order_item' } })

    const json = [];

    orders.forEach((o) => {
        o.order_item.forEach((i) => {
            json.push({
                ID: o.id,
                Name: o.name,
                Email: o.email,
...
const Chart = async (req, res) => {
    ...
    const result = await data.query(`
        SELECT
        TO_CHAR(o."createdAt", 'YYYY-MM-DD') as date,  -- Use double quotes for createdAt
        REPLACE(TO_CHAR(TRUNC(sum(i.price * i.quantity)), 'FM999G999G999'), ',', '') as sum
        FROM orders o
        JOIN order_items i on o.id = i.order_id
        GROUP BY TO_CHAR(o."createdAt", 'YYYY-MM-DD')  -- Use double quotes for createdAt
        ORDER BY TO_CHAR(o."createdAt", 'YYYY-MM-DD') ASC;  -- Use double quotes for createdAt
    `, { type: data.QueryTypes.SELECT });
...
module.exports = { Orders, Export, Chart };