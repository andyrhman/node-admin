...
export const Users = async (req: Request, res: Response) => {
    ...
    let result = await repository.paginate(page, take, ['role', 'role.permissions']);
...
export const CreateUser = async (req: Request, res: Response) => {
    ...
    const emailExists = await myPrisma.user.findUnique({ where: { email: body.email.toLowerCase() } });
    const usernameExists = await myPrisma.user.findUnique({ where: { username: body.username.toLowerCase() } });
    ...
    const checkRole = await myPrisma.role.findUnique({ where: { id: body.role_id } });
    if (!checkRole) {
        return res.status(409).send({
...
export const UpdateUser = async (req: Request, res: Response) => {
    ...
    const existingUser = await myPrisma.user.findUnique({ where: { id: req.params.id } });
    ...
    if (req.body.role_id) {
        const role = await myPrisma.role.findUnique({ where: { id: req.body.role_id } });
        if (!role) {
            return res.status(404).send({ message: 'Role not found' });
        }
        existingUser.roleId = req.body.role_id;
    }

    await myPrisma.user.update({ where: { id: req.params.id }, data: { ...existingUser } });

    const { password, ...data } = await myPrisma.user.findUnique({ where: { id: req.params.id }, include: { role: true } });
...
export const GetUser = async (req: Request, res: Response) => {
    ...
    const { password, ...user } = await myPrisma.user.findUnique({ where: { id: req.params.id }, include: { role: true } });
...
export const DeleteUser = async (req: Request, res: Response) => {
    ...
    await myPrisma.user.delete({ where: { id: req.params.id } });
